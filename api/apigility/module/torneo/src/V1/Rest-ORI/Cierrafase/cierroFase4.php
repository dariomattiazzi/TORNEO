...
class cierroFase4 {
	...
	public function getCierroFase4($torneo_id, $categoria_id, $zona_id, $fase_id){
		...
		// print_r($arr_equipo_categoria); die;
		...
		foreach ($arr_equipos as $key => $row) {
			...
			// echo "PARTIDO : " .$eq1 ." VS ".$eq2."\n";
			// echo "     GOLES EQ1: " .$goles_eq1 ." - GOLES EQ2: " .$goles_eq2 ." /// PENALES EQ1: " .$penales_eq1 ." - PENALES EQ2: " .$penales_eq2 ."\n";
			// // die;


			if($eq2 == '9999'){
				$arr_ganadoresCuartos[$eq1] = array('equipo_id' => $eq1);
			}else{
				if($goles_eq1 > $goles_eq2){
					$arr_ganadoresCuartos[$eq1] = array('equipo_id' => $eq1);
				}elseif($goles_eq2 > $goles_eq1){
					$arr_ganadoresCuartos[$eq2] = array('equipo_id' => $eq2);
				}elseif($goles_eq1 == $goles_eq2){
					$penales_eq1 = $row['fixture_penales_eq1'];
					$penales_eq2 = $row['fixture_penales_eq2'];
					// echo "PARTIDO : " .$eq1 ." VS ".$eq2."\n";
					// echo "     GOLES EQ1: " .$goles_eq1 ." - GOLES EQ2: " .$goles_eq2 ." /// PENALES EQ1: " .$penales_eq1 ." - PENALES EQ2: " .$penales_eq2 ."\n";
					echo $penales_eq1 ."----".$penales_eq2; //die;
					if($penales_eq1 > $penales_eq2){
						$arr_ganadoresCuartos[$eq1] = array('equipo_id' => $eq1);
					}
					if($penales_eq2 > $penales_eq1){
						$arr_ganadoresCuartos[$eq1] = array('equipo_id' => $eq2);
					}
				}
			}
		}
		...
		foreach ($arr_ganadoresCuartos as $clave => $fila) {
		...
		$select = $sql->select();
		$select->from('fixture');
		$select->where(array('fixture_zona_id' => $zona_id, 'fixture_fase_id' => "4" ));
		$selectString = $sql->getSqlStringForSqlObject($select);
		$results   = $this->adapter->query($selectString, Adapter::QUERY_MODE_EXECUTE);
		$Zc = $results->toArray();

		if (!empty($Zc)) {
			//			echo "ya existe la FASE 4 para la zona Campeonato";
			$id = $Zc['0']['zona_id'];

			$sql = new Sql($this->adapter);
			$delete = $sql->delete();
			$delete->from('fixture');
			$delete->where(array('fixture_zona_id' => $zona_id, 'fixture_fase_id' => "5"));
		...
		for ($i = 0; $i <= 1; $i++) {
			...
			$fixture_equipo_id2 = $arr_cuartos[3-$i]['equipo_id'];
