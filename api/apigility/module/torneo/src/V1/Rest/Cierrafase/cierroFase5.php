...
class cierroFase5 {
	...
	public function getCierroFase5($torneo_id, $categoria_id, $zona_id, $fase_id){
		$cantidaEquipoXCat = 0;
		$arr_general = array();
		...
		$arr_ganadoresSemis = array();
		$arr_perdedoresSemis = array();
		foreach ($arr_equipos as $key => $row) {
			...
			if($eq2 == '9999'){
				$arr_ganadoresSemis[$eq1] = array('equipo_id' => $eq1);
			}else{
				if($goles_eq1 > $goles_eq2){
					$arr_ganadoresSemis[$eq1] = array('equipo_id' => $eq1);
					$arr_perdedoresSemis[$eq2] = array('equipo_id' => $eq2);
				}elseif($goles_eq2 > $goles_eq1){
					$arr_ganadoresSemis[$eq2] = array('equipo_id' => $eq2);
					$arr_perdedoresSemis[$eq1] = array('equipo_id' => $eq1);
		...
		// print_r($arr_ganadoresSemis); die;
		// echo "\n";
		// print_r($arr_perdedoresSemis);
		// die;
		$e = 0;
		foreach ($arr_ganadoresSemis as $key => $row) {
			$arr_ganadoresSemis[$e] = $key;
			$e++;
		}
		...
		$select = $sql->select();
		$select->from('fixture');
		$select->where(array('fixture_zona_id' => $zona_id, 'fixture_fase_id' => "7" ));
		$selectString = $sql->getSqlStringForSqlObject($select);
		$results   = $this->adapter->query($selectString, Adapter::QUERY_MODE_EXECUTE);
		$Zc = $results->toArray();
...
//		print_r($Zc); die;
		...
		if (!empty($Zc)) {
//			echo "ya existe la FASE 7 para la zona Campeonato"; die;
			$id = $zona_id;

			$sql = new Sql($this->adapter);
			$delete = $sql->delete();
			$delete->from('fixture');
			$delete->where(array('fixture_zona_id' => $id, 'fixture_fase_id' => "7"));
		...
		$fixture_fecha = '1';
		$fixture_equipo_id1 = $arr_ganadoresSemis[0];
		$fixture_equipo_id2 = $arr_ganadoresSemis[1];
		$fixture_fase_id = 7;
		...
		$select = $sql->select();
		$select->from('fixture');
		$select->where(array('fixture_zona_id' => $zona_id, 'fixture_fase_id' => "6" ));
		$selectString = $sql->getSqlStringForSqlObject($select);
		$results   = $this->adapter->query($selectString, Adapter::QUERY_MODE_EXECUTE);
		$Zc = $results->toArray();
		...
		if (!empty($Zc)) {
			//			echo "ya existe la FASE 6 para la zona Campeonato";
			$id = $zona_id;
			$sql = new Sql($this->adapter);
			$delete = $sql->delete();
			$delete->from('fixture');
			$delete->where(array('fixture_zona_id' => $id, 'fixture_fase_id' => "6"));
			$deleteString = $sql->getSqlStringForSqlObject($delete);
			$results = $this->adapter->query($deleteString, Adapter::QUERY_MODE_EXECUTE);
		}

		$e = 0;
		foreach ($arr_perdedoresSemis as $key => $row) {
			$arr_perdedoresSemis[$e] = $key;
			$e++;
		}
		...
		$fixture_zona_id = $zona_id;
		$fixture_fecha = '1';
		$fixture_equipo_id1 = $arr_perdedoresSemis[0];
		$fixture_equipo_id2 = $arr_perdedoresSemis[1];
		$fixture_fase_id = 6;
		$fixture_cancha_id = '999';
		$fixture_turno_id = '999';
		$fixture_goles_eq1 = null;
